...
final class AppState: ObservableObject {
    ...
    func handleIncomingURLs(_ urls: [URL]) {
        guard !isCleaning, let pdfURL = urls.first(where: Self.isPDF(_:)) else {
            return
        }
    ...
    func setSelectedPDF(from url: URL?) {
        guard !isCleaning, let url, Self.isPDF(url) else {
            return
        }
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private let defaultWindowSize = NSSize(width: 620, height: 350)
    ...
    private func makeMainMenu() -> NSMenu {
        ...
        // Keep the compact menus while restoring the standard window shortcut.
        let minimizeItem = NSMenuItem(title: "", action: #selector(NSWindow.performMiniaturize(_:)), keyEquivalent: "m")
        minimizeItem.keyEquivalentModifierMask = .command
        minimizeItem.isHidden = true
        minimizeItem.allowsKeyEquivalentWhenHidden = true
        ...
        let languageMenu = NSMenu(title: L10n.languageMenuTitle)
        languageMenuItem.submenu = languageMenu

        let automaticItem = NSMenuItem(
            title: L10n.automaticLanguageMenuTitle,

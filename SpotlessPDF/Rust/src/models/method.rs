        ...
        match self {
            Method::Wuolah(content_list, to_delete) => {
                ...
                for (i, page) in vector.iter().enumerate() {
                    ...
                    // Preserve annotations: handwritten solutions may live entirely in them.
            ...
            Method::Naive => {
                ...
                for page in &pages {
                    ...
                    // An unrecognized page is not evidence of an advertisement.
                    // This also preserves blank pages without Resources or Contents.
                    if matches!(page_type, page_type::PageType::Idk) {
                        continue;
                    }
                    let mutable_page = doc.get_object_mut(*page.1).unwrap().as_dict_mut().unwrap();
                ...
                for page in &pages {
                    if matches!(page_type::PageType::get_page_type(doc, page.1).unwrap_or_default(), page_type::PageType::Idk) {
                        continue;
                    }
                    // remove the logo
                    let _ = remove_logo(doc, page.1);

                    // Annotations are user content, not evidence of advertisements.
                }
...
pub fn get_xobjs<'a>(doc: &'a Document, page: &ObjectId) -> Result<&'a Dictionary, Box<dyn Error>> {
    ...
    let resource_dict: &Dictionary = if resource.1.is_empty() {
        resource.0.ok_or("Page has no resources")?
    } else {
